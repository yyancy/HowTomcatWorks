...
1. Catalina携带args参数执行process方法
    1.设置catalina_base和catalina_home参数
    2. 解析相关args参数,执行相关方法
    
2. 创建Digester,解析conf/server.xml配置文件内容.解析完成会生成Server对象并赋值到Catalina.server中.
3. 调用server的初始化方法.server.initialize();
   1. 初始化包含的所有service. service.initialize();
        1. service初始化包含的所有connector,例如http,https. connectors.initialize();

4. 并启动server server.start()
    1. 触发before_start,start生命周期事件,相关注册的监听器执行
    2. 启动所有的service. services).start()
        1. 触发before_start,start生命周期事件,相关注册的监听器执行
        2. 启动所有的container. container.start(). 容器的启动见下文
        3. 启动所有的connector. connector.start()
        4. 触发after_start生命周期事件,相关注册的监听器执行
    
    3. 触发after_start生命周期事件,相关注册的监听器执行    
    
5. 调用server.await()保持程序运行.
6. 若退出,则服务结束,调用server.stop()方法清理相关对象,结束服务.

## Connector 运行流程

1. 调用start()方法,会启动后台监控socket线程
2. 执行run()方法, 对接收的请求,转交到HttpProcessor的process方法处理
3. 进行相关初始化操作,之后parse相关元信息. 
4. 调用connector关联的container的invoke处理相关请求.(一般为Engine)

## 容器启动 engine.start()

1. 触发BEFORE_START_EVENT事件,添加mapperClass.
2. 